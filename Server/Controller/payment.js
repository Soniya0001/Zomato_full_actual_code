...
router.post("/orders", async (req, res) => {
    try {
        ...
        instance.orders.create(options, (errors, orders) =>{
            if(error){
                console.log(error);
...
router.post("/verify", async (req, res) =>{
    try{
         ...
         const expectedSign = Crypto.createHmac("sha256", process.env.KEY_SECRET).update
         (sign.toString()).digest("hex");

         if(razorpay_signature === expectedsign){
            return res.status(200).json({ message: "Payemt Verified"});
        
         }else {
            return res.status(400).json({ message: "Payemt Failed"});
         }
